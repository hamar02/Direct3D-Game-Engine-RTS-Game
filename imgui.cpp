...
IMGUI_API void ImGuiIO::SetCursorTexture(int texture, ImVec2 m_size)
{
	ImGuiContext& g = *GImGui;

	const ImVec2 pos = g.IO.MousePos;
	ImVec2 s = ImVec2(0, 0);
	const ImTextureID tex_id = (ImTextureID)texture;
	...
	if (g.IO.MouseDrawCursor && g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor, &offset, &size, &uv[0], &uv[2]))
	{
		...
		if (m_size == s)
			...
			g.OverlayDrawList.AddImage(tex_id, pos, pos + m_size * sc, uv[0], uv[1], IM_COL32(255, 255, 255, 255)); // White fill
